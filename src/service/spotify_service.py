...
def first_time_spotify_authorization(code, username):
    ...
    if response.status_code != 200:
        raise SpotifyException(
            f"Received response with status code {response.status_code} from Spotify auth endpoint"
        )
...
def get_spotify_tracks(track_dictionary_list, access_token):
    ...
    for track_dictionary in track_dictionary_list:
        spotify_track = search_for_song(
            track_dictionary["artist"], track_dictionary["title"], access_token
        )
...
def search_for_song(artist, song_title, access_token):
    ...
    search_response = requests.get(
        SEARCH_URL + params, headers={"Authorization": f"Bearer {access_token}"}
    )

    if search_response.status_code != 200:
        LOGGER.exception(
            f"Received {search_response.status_code} response from Spotify search"
        )
        raise SpotifyException(
            f"Received {search_response.status_code} response from Spotify search"
        )
...
def get_spotify_track_attributes(spotify_response):
    try:
        ...
        LOGGER.exception("Error grabbing Spotify song attributes")
...
def get_spotify_id(username, access_token):
    ...
    user_response_raw = requests.get(
        GET_USER_URL, headers={"Authorization": f"Bearer {access_token}"}
    )
    user_response = user_response_raw.json()

    if user_response_raw.status_code != 200:
        raise SpotifyException(
            f"Got response code {user_response_raw.status_code} trying to get ID for "
            + f'user "{username}"'
        )
    try:
...
def create_playlist(access_token, spotify_user_id):
    ...
    request_body = {"name": "Your Awesome Playlist"}
    LOGGER.info(f"\n\nMaking request to {create_playlist_url}\n\n")
    playlist_response_raw = requests.post(
        create_playlist_url,
        json=request_body,
...
def add_songs_to_playlist(access_token, playlist_id, spotify_tracks):
    ...
    request_body = {"uris": [track.get("uri") for track in spotify_tracks]}
    LOGGER.info(f"\n\nMaking request to {add_songs_url}\n\n")
    add_songs_response_raw = requests.post(
        add_songs_url,
        json=request_body,
        headers={"Authorization": f"Bearer {access_token}"},
    )
    ...
    if add_songs_response_raw.status_code not in (200, 201):
        raise SpotifyException(
            f"Received {add_songs_response_raw.status_code} response from Spotify playlist"
